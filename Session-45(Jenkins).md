...
If you want to configure in CI pipeline, you need to create this file, "sonar-scanner" is the command to read sonar-project.properties to scan the code and uploading the results. Go through the code in VS. Push to the git and run the CI pipeline in jenkins UI. Instead of giving username and password in sonar-project properties, you can generate token in sonarqube and give it as 'sonar.login=token'
...
- Create a feature branch in catalogue, git branch -M feature-1
- Configure multi-branch pipeline, Keep 1 Jenkinfile and also create one folder Roboshop in jenkins, inside this create catalogue as multibranch pipeline
- Configure Jenkins shared libraries

### Jenkins shared library (Centralized pipeline)
Go through the code of "Roboshop-shared-library" in VS, here we can write our pipelines with minimum code. Inside the vars folder, we created a centralized pipeline for nodejs and named as "nodejsVM.groovy" this CI pipeline will work for all nodejs and VM projects, we need to inform this Jenkins shared library repo to the jenkins by going to the manage jenkins/system/global pipeline libraries/add here, default version should be main and name (Any-name)/project repo should be git URL. Refer this library in jenkins pipeline using @Library('roboshop-shared-library') and send parameters like what type of application and component using pipelineDecission. Similarly we can also create for javaVM/pythonVM/goVM etc. Whatever files there in "Roboshop-shared-library" are called as Shared libraries (or) Centralized Pipelines. Below is the overall process.
