...
- / ---> Root folder (or) Root directory. "-" ---> File; r ---> Read; w ---> Write; x ---> Execute
- ls -l ---> Lengthy format in alphabetical order.
- ls -lr ---> Lengthy format in reverse alphabetical order.
- ls --help (or) command --help ---> Will provide all commands regarding ls, however we dont use all.
...
- mkdir ---> Creates empty folder. Blue colour is folder and white colour is file.
...
- rm <file-name> (or) rm -rf <file-name> ---> Deletes the file.
...
### Move command
If you use mv command with in the same folder, it works as rename also. So mv command can also be used for renaming and Usage is "mv sample /Videos"
...
- Curl ---> Downloads the content directly on the terminal without downloading file, we use this more. For example take your any session URL from git and try it. Make sure use only RAW URL of the session. Usage is "curl <RAW_URL_of_the_session>"
...
- Awk will divide the data based on columns.

### Head and Tail commands
- head <file-name> ---> First 10 lines
- tail <file-name> ---> Last 10 lines
- To get specific ---> head -n 1 <file-name> (or) tail -n 1 <file-name>
...
- We use Ctrl+F ---> To find (or) search (or) replace in windows, but to replace in linux ":s/<word-to-find>/<word-to-replace>" replaces the word where your cursor is. This will replace only first occurance in the line. To replaces all use "/g", if you want any particular line then ":2s" s=substitute. Example:- ":%s/sbin/SBIN/g" ===> %s means all occurances.
