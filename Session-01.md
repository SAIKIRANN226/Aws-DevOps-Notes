...
- Server ---> Is used for Hosting or installing web applications to serve users (or) clients. Not only installing applications but also we can listen songs, watch movies etc. But main purpose is for installing applications.
...
- "ssh -i <Path-to-Privatekey> Username@IP" ===> Usage is {ssh -i saikiran.pem ec2-user@123.23.234.5} and this command should be entered in the gitbash in pwd "c/Users/saikiran"
...
- Absolute path ---> ssh -i /c/users/user/saikiran.pem ec2-user@IP (If you dont know where you are then you can give the location from the starting)
